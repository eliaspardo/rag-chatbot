...
def get_parent_runs(
    client: "MlflowClient", exp_id: str, max_results: int
) -> List:
...
def format_child_runs_table(
    ...
    """Markdown table: Q-ID, Status, metric scores, Question (60 chars), extra fields (120 chars)."""
    ...
    for run in sorted_runs:
        ...
        status = tags.get("status", "")
        scores = [
            f"{metrics[m]:.3f}" if m in metrics else "" for m in metric_names
        ]
    ...
    sep = "| " + " | ".join("-" * w for w in col_widths) + " |"
    lines = (
        [header, "", _fmt_row(headers, col_widths), sep]
...
def cmd_list(args) -> None:
    ...
    if exp is None:
        print(
            f"ERROR: Experiment '{experiment}' not found at {uri}", file=sys.stderr
        )
        sys.exit(1)
...
Examples:
  ...
  %(prog)s show deepeval-2026-04-08-11-56-56 --fields "actual output,expected output,failure"
    ...
    show_p = subparsers.add_parser("show", help="Show child runs for a parent run")
    show_p.add_argument("run", metavar="RUN_NAME_OR_ID")
    show_p.add_argument("--tracking-uri", default=None, dest="tracking_uri")
    show_p.add_argument("--experiment", default=None)
    show_p.add_argument(
        "--status",
