...
def update_document_status(
    doc_hash: str, document_status: DocumentStatus, base_url: str
) -> None:
    try:
        request_body = SetDocumentStatusRequest(status=document_status)
        response = requests.put(
            f"{base_url}/documents/{doc_hash}/status", json=request_body.model_dump()
        )
    except Exception:
        raise
    response.raise_for_status()
...
def get_documents(base_url: str) -> List[DMSDocument]:
    try:
